...
export default function CarList({ isEdit = false }: CarListInterface) {
  ...
  const [convertKeyboardStatys, setConvertKeyboardStatys] =
    ...
    try {
      ...
      if (isEdit && carId) {
        await carApi.editCar({
          ...
          images: form.fileInputs.carImages.objectFiles.map(
            (file) => file.base64
          ),
        });
        navigate("/");
    ...
    <>
      ...
      <DragAndDrop
        ...
        handleDragLeave={(e: any) =>
          form.fileInputs.carImages.handleDragLeave(e)
        }
      ...
      >
        <div className="pageHeader">
          <PathToPage
            props={[
              ...
              [
                `Car ${isEdit ? "edit" : "create"}`,
                `${isEdit ? `${CAR_PAGE.edit}/${carId}` : CAR_PAGE.add}`,
              ],
            ]}
          />
        ...
        <div className="addCarPage">
          <div className="imgContainer">
            <div className="innerImageContainer">
              <Galery
                src={form.fileInputs.carImages.objectFiles.map(
                  (item) => item.url
                )}
                defaultImg={NoPhoto}
                isUrl
        ...
        {convertKeyboardStatys && (
          <PopupConvertKeyboard close={() => setConvertKeyboardStatys(false)} />
        )}
